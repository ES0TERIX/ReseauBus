...
{
    /// <summary>
    /// Formulaire principal de simulation - Implémente IObserver
    /// </summary>
    public partial class FormSimulation : Form, IInterfaceUtilisateur
    {
        ...
        {
            this.Text = "Simulation du réseau de bus";
        ...
        private void CreerPanneauxSimulation(TableLayoutPanel mainPanel)
        {
            ...
            for (int i = 0; i < _configurations.Count && i < 4; i++)
            {
                ...
                if (config.TypeVisualisation == TypeVisualisation.Graphique)
                {
                    panneau = new PanneauGraphique(simulation, config);
                }
                else
                {
